...
for bits in [7, 10, 13, 16, 19, 22]:
    simulate.BITs = bits
    torch.manual_seed(0)
    mb2 = CNNModelBaseline().to(dev)   # fresh baseline init to copy from (same seed -> same weights)
    mq = CNNModelQ().to(dev)
    sync_models(mb2, mq)
    losses, pmax, wmax = train_run(bits, mq)
    finite = [x for x in losses if math.isfinite(x)]
    row = ' '.join('%7.4g' % losses[c] for c in CKPTS)
    sys.stderr.write('SW BITs=%-2d   %s  min %.4f  |w|max %.3g  ptrunc_max %.3g (2^%.1f, %.1f%% of 2^53, %.1f%% of 2^56)  nonfin %d\n'
        % (bits, row, (min(finite) if finite else float('nan')), wmax, pmax,
