...
# CONFIRMATION: full 12-epoch training at the candidate minimum ring widths for the BITs=10 floor.
# The short sweep showed the limiter is OVERFLOW (payload P < 2^(l-1)), not truncation noise. The sim
# only faithfully models a Z_{2^l} ring while P < 2^(l-1) (all other ops stay exact int64); once the
# overflow flag fires the run no longer represents a real dtype. So the trustworthy minimum is the
# smallest l that trains to baseline for the FULL run with the flag never firing. l=40 is a safety anchor.
...
log('RC cached %d train / %d test  BITs=%d S=2^%d A=%d  epochs=%d' %
...
for L in RING_LS:
    ...
    peak = {'p': 0.0}; orig = simulate.ring_truncate
    ...
    ovf_ever = False
    log('RC === RING_BITS=%d ===' % L)
    log('RC ep | base t1 | quant t1 | gap | paramRMSE | payloadP 2^ | overflow? | nonfin')
    for ep in range(1, EPOCHS + 1):
        mb.train()
        for x, tgt in train:
            ob.zero_grad(); l_ = ce(mb(x.to(torch.float32)), tgt); l_.backward(); ob.step()
        mq.train(); peak['p'] = 0.0; simulate.ring_truncate = spy; nonfin = 0
        try:
            for x, tgt in train:
                ...
                oq.step(); nonfin += int((~torch.isfinite(out)).sum().item())
        finally:
        ...
        ovf = Pb >= (L - 1); ovf_ever = ovf_ever or ovf
        log('RC e%-2d | %5.2f | %5.2f | %+6.2f | %6.3f%% | 2^%5.2f | %-8s | %d'
            % (ep, b1, q1, q1 - b1, 100 * rmse, Pb, 'OVERFLOW' if ovf else '-', nonfin))
    log('RC summary RING_BITS=%d : %s  (faithful=%s)' %
        (L, 'trained' if q1 > 30 else 'DEGRADED', 'NO-overflowed' if ovf_ever else 'yes'))
